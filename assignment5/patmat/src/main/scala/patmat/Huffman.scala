...
object Huffman {
   ...
   */
    def until(check_single: List[CodeTree]=> Boolean, combine_node: List[CodeTree]=>List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = {

      if(check_single(trees)) trees
      else until(check_single,combine_node)(combine_node(trees))
    }
   ...
   */
    def convert(tree: CodeTree): CodeTable ={def add(chars: List[Char], bit: Bit, accu: CodeTable): CodeTable = {
