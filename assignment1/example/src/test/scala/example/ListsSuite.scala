 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of when list has negative numbers") {
    assert(sum(List(-1,0,2)) === 1)
  }
  test("max of numbers when list has negative elements") {
    assert(sum(List(-2,-3,-13)) === -2)
  }

  test("max of numbers when list has repeated numbers") {
